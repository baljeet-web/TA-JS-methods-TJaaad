...
function addThreeToArray(numbers){
  for (var number of numbers){
    addThreeArr.push(addThree(number))
  }
  return addThreeArr
}
...
function addFiveToArray(numbers){
  for (var number of numbers){
    addFiveArr.push(addFive(number))
  }
  return addFiveArr;
}
...
let filterAdult = function(array){
  let newArr = [];
  for (var i of array){
    if(isAdult(i) ==true){
      newArr.push(i);
